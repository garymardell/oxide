module Graphene
  module Validation
    class ContextVisitor < Graphene::Language::Visitor
      property context : Validation::Context

      def initialize(@context : Validation::Context)
      end

      def enter(node : Graphene::Language::Nodes::SelectionSet)
      ...
      def enter(node : Graphene::Language::Nodes::Field)
        definition = nil
        type = nil
      ...
      def enter(node : Graphene::Language::Nodes::InlineFragment)
      ...
      def enter(node : Graphene::Language::Nodes::ListType)
      end
      ...
      def enter(node : Graphene::Language::Nodes::ObjectField)
      ...
      def enter(node : Graphene::Language::Nodes::EnumTypeDefinition)
      end

      def enter(node)
      end

      def leave(node : Graphene::Language::Nodes::SelectionSet)
        context.parent_type_stack.pop
      end

      def leave(node : Graphene::Language::Nodes::Field)
        context.field_definition_stack.pop
        context.type_stack.pop
      end

      def leave(node : Graphene::Language::Nodes::OperationDefinition)
        context.type_stack.pop
      end
      ...
      private def field_definition(parent_type : Graphene::Types::ObjectType, field_name)
      ...
      private def named_type(type)
        case type
        when Graphene::Types::ListType
          named_type(type.of_type)
        when Graphene::Types::NonNullType
          named_type(type.of_type)
        else
          type
        end
